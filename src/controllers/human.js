...
async function detect(inputFile) {
  ...
  logger.debug('Human:', human.version, 'TF:', tf.version_core); // eslint-disable-line no-console
  ...
  logger.debug('loaded input file:', inputFile, 'resolution:', tensor.shape); // eslint-disable-line no-console
  const result = await human.detect(tensor); // run detection; will initialize backend and on-demand load models
  logger.debug(result); // eslint-disable-line no-console
}
